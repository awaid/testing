...
#define kDefaultPopOverWidth 347
#define MG_DEFAULT_SPLIT_POSITION		257	// default width of master view in UISplitViewController.
...
#define MG_DEFAULT_CORNER_COLOR			[UIColor clearColor]	// default color of intruding inner corners (and divider background).
...
#define kWidthOfPopOver 290
#define kHeightOfPopOver 287
...
{
    return CGSizeMake(347, 248);
}

- (void)reconfigureForMasterInPopover:(BOOL)inPopover
{
	_reconfigurePopup = NO;
    self.popoverViewController.view.frame=CGRectMake(0, 0, kDefaultPopOverWidth, kHeightOfPopOver);

    if(!self.popoverViewController)
	{
        self.popoverViewController = [[VSPopoverViewController alloc] init];
    }
	...
	if (inPopover && !_hiddenPopoverController && !_barButtonItem) {
		...
		[self.popoverViewController viewWillDisappear:NO];
		_hiddenPopoverController = [[UIPopoverController alloc] initWithContentViewController:self.popoverViewController];
        _hiddenPopoverController.popoverContentSize=CGSizeMake(kWidthOfPopOver, kHeightOfPopOver);

		[self.popoverViewController viewDidDisappear:NO];
		//[_hiddenPopoverController presentPopoverFromRect:CGRectMake(0, 0, 347, 248) inView:self.view permittedArrowDirections:UIPopoverArrowDirectionUp animated:YES];
		// Create and configure _barButtonItem.
        
        if(!_barButtonItem)
        {
            UIButton *settingsView = [[[UIButton alloc] initWithFrame:CGRectMake(0, 0, 50, 29)] autorelease];
            [settingsView addTarget:self action:@selector(showPopover:) forControlEvents:UIControlEventTouchUpInside];
            [settingsView setBackgroundImage:[UIImage imageNamed:@"value-stream-slices_17.png"] forState:UIControlStateNormal];
            _barButtonItem = [[UIBarButtonItem alloc] initWithCustomView:settingsView];
                              /*initWithTitle:NSLocalizedString(@"Master", nil)
		...
		if (_delegate && [_delegate respondsToSelector:@selector(splitViewController:willHideViewController:withBarButtonItem:forPopoverController:)]) {
			[(NSObject <MGSplitViewControllerDelegate> *)_delegate splitViewController:self 
																willHideViewController:self.popoverViewController 
		...
		//[_hiddenPopoverController presentPopoverFromRect:CGRectZero inView:self.view permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
...
//			[(NSObject <MGSplitViewControllerDelegate> *)_delegate splitViewController:self 
//																willShowViewController:self.popoverViewController 
	...
	if (_hiddenPopoverController && !(_hiddenPopoverController.popoverVisible)) {
		...
		if (_delegate && [_delegate respondsToSelector:@selector(splitViewController:popoverController:willPresentViewController:)]) {
			[(NSObject <MGSplitViewControllerDelegate> *)_delegate splitViewController:self 
															 ...
															 willPresentViewController:self.popoverViewController];
		}
...
#pragma mark - AutoRotation disabling methods
